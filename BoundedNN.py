...
class Base(nn.Module):
    ...
    def forward(self, x):
        ...
        max_val = 1
        min_val= 0
        return F.relu(max_val - min_val - F.relu(max_val - x)) + min_val
...
rmse = np.sqrt(mean_squared_error(output_t_np, predictions))*5.9
print("RMSE:", rmse)
...
df = df.T
df.to_csv(f'results_bounded_{choice}_test_{N_end}.csv', index=True, header=True)
...
df = df.T
df.to_csv(f'results_bounded_{choice}_train_{N_end}.csv', index=True, header=True)