...
ini_weights = os.path.join("warmup_model_SSN.pth")
...
class Net_state_space(nn.Module):
    def __init__(self, num_neuron=3, dropout_p = 0):
        super(Net_state_space, self).__init__()
        self.fc1_1 = nn.Linear(1, 1)
        self.fc1_2 = nn.Linear(3, num_neuron)
        self.fc3 = nn.Linear(1, 1)
        self.fc4 = nn.Linear(num_neuron, 1)
    def forward(self, x):
        x, y, z = x[:, :1], x[:, 1:2], x[:, 2:]
        x_1 = F.relu(self.fc1_1(x))
        y = torch.cat((x, y, z), dim=1)
        y_1 = F.relu(self.fc1_2(y))
        x = self.fc3(x_1) + self.fc4(y_1)
        return x
...
def choose_lr(dataset, param_grid, n_splits=5):
    ...
    for params in param_grid:
        ...
        for train_idx, val_idx in kf.split(dataset):
            ...
            model = Net_state_space(num_neuron=num_neuron).to(device)
...
model = Net_state_space(num_neuron=param_grid[0]['num_neuron']).to(device)
...
df = df.T
df.to_csv(f'results_SSN_{choice}_test_{N_end}.csv', index=True, header=True)
...
df = df.T
df.to_csv(f'results_SSN_{choice}_train_{N_end}.csv', index=True, header=True)