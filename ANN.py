...
param_grid = [{'num_neuron': best_neuron, 'epochs': epoch, 'lr': best_lr}]
...
df = df.T
df.to_csv(f'results_nn_{choice}_test_{N_end}.csv', index=True, header=True)
...
df = pd.DataFrame({
    ...
    'r2': r2_list})
df = df.T
df.to_csv(f'results_nn_{choice}_train_{N_end}.csv', index=True, header=True)
